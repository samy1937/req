...
func (r *Req) Do(method, rawurl string, vs ...interface{}) (resp *Resp, err error) {
	...
	for _, v := range vs {
		...
		case *bodyJson:
			fn, err := setBodyJson(req.Request, resp, r.jsonEncOpts, vv.v)
			if err != nil {
				return nil, err
			}
			delayedFunc = append(delayedFunc, fn)
		case *bodyXml:
			fn, err := setBodyXml(req.Request, resp, r.xmlEncOpts, vv.v)
			if err != nil {
				return nil, err
			}
			delayedFunc = append(delayedFunc, fn)
		...
		case []byte:
			setBodyBytes(req.Request, resp, vv)
		case bytes.Buffer:
			setBodyBytes(req.Request, resp, vv.Bytes())
		case *http.Client:
			resp.client.HTTPClient = vv
		case FileUpload:
		...
		case Host:
			req.Host = string(vv)
		case io.Reader:
			fn := setBodyReader(req.Request, resp, vv)
	...
	if len(uploads) > 0 && (req.Method == "POST" || req.Method == "PUT") { // multipart
		...
		multipartHelper.Upload(req.Request)
		...
		if !formParam.Empty() {
			if req.Body != nil {
				...
				setBodyBytes(req.Request, resp, []byte(formParam.Encode()))
				setContentType(req.Request, "application/x-www-form-urlencoded; charset=UTF-8")
			}
		}
	}
